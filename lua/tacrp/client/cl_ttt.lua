...
if !TTT2 then
    hook.Add("TTTSettingsTabs", "TacRP", function(dtabs)
        ...
        dgui3:ControlHelp("Bullets will be hitscan up to a certain range depending on muzzle velocity.")
        dgui3:CheckBox("Enable Holstering", "tacrp_holster")
        dgui3:ControlHelp("Play a holster animation before pulling out another weapon. If disabled, holstering is instant.")
        dgui3:CheckBox("Enable Shotgun Patterns", "tacrp_fixedspread")
        dgui3:ControlHelp("Shotgun pellets uses a pattern that covers the spread area for more consistency.")
        dgui3:CheckBox("Enable Pattern Randomness", "tacrp_pelletspread")
        dgui3:ControlHelp("Add random spread onto the pattern. Does not affect total spread. If disabled, shotgun patterns become completely static.")
        dgui3:CheckBox("Enable Pattern Randomness", "tacrp_pelletspread")
        dgui3:CheckBox("Custom Armor Penetration", "tacrp_armorpenetration")
        dgui3:ControlHelp("Weapons use defined piercing and shredding stats to calculate damage when hitting players with HL2 suit armor, instead of using the standard 20% damage. This generally increases the weapons' effectiveness against armor.\nCompatible with Danger Zone Entities' armor.")
        dgui3:CheckBox("Enable Scope Glint", "tacrp_glint")
        dgui3:ControlHelp("Scopes show a visible glint. Glint size is dependent on angle of view, scope magnification and distance, and is bigger when zoomed in.")
        dgui3:CheckBox("Enable Blinding Flashlights", "tacrp_flashlight_blind")
        dgui3:ControlHelp("Flashlight glare will obscure vision based on distance and viewing angle. Effect is more significant on scopes. If disabled, glare sprite will be visible but not grow in size.")
